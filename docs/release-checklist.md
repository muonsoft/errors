...
- [ ] `CHANGELOG.md` has either a non-empty exact planned version section or non-empty
      `[Unreleased]` section.
- [ ] README and migration guidance are current.
...
bash scripts/prepare-release.sh 0.5.1 --check-only
...
2. Select `main` and enter a `v`-prefixed SemVer version, for example `v0.5.1`.
...
```bash
gh release view v0.5.1 --repo muonsoft/errors
git ls-remote --tags origin refs/tags/v0.5.1

tmp_dir=$(mktemp -d)
cd "$tmp_dir"
go mod init release-smoke
GOTOOLCHAIN=local go get github.com/muonsoft/errors@v0.5.1
go list -m github.com/muonsoft/errors
```
